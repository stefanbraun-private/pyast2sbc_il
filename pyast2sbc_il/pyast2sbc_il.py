...
It should generate a minimal code skeleton as starting point for your next Instruction List project.

Current state of project: not much changed in original code
(hmm, there is much to do and SAIA(R) PCD Instruction List (IL) has many differences to x64 assembler...)

WARNING: This project is only my test and comes without any warranties.

"pyast2sbc_il": Copyright (c) 2018 Stefan Braun
("pyast2sbc_il" is loosely based on "pyast64" Copyright (c) 2017 Ben Hoyt)

pyast2sbc_il is loosely based on pyast64. Special thanks to Ben Hoyt for his inspiring idea
and good documentation on his website: http://benhoyt.com/writings/pyast64/
...
class Compiler:
    """The main Python AST -> SAIA(R) PCD Instruction List compiler."""
    ...
    def header(self):
        self.asm.comment('code skeleton generated by "pyast2sbc_il"')
        self.asm.comment('')
